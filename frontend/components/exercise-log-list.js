...
function ExerciseLogSection({ title, logs, emptyText }) {
  if (!logs.length) {
    return (
        ...
        {logs.map((log) => (
          <div key={log.id} className="glass-panel rounded-[24px] p-5 shadow-glass">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Workout</p>
  ...
  return (
    <div className="space-y-5">
      <ExerciseLogSection
        ...
        logs={manualLogs}
        emptyText="暫時沒有手動輸入的運動紀錄。"
      />
      <ExerciseLogSection
        title="Apple Health Imports"
        logs={importedLogs}
        emptyText="暫時沒有 Apple Health 匯入的運動紀錄。"
      />
    </div>
  );
}
